...
class UserInteraction {
     ...
     */
    async click(element, timeout = process.env.QMATE_CUSTOM_TIMEOUT || 30000) {
     ...
     */
    async clickAndRetry(element, timeout = process.env.QMATE_CUSTOM_TIMEOUT || 30000, retries = 3, interval = 5000) {
        const vl = this.vlf.initLog(this.click);
        try {
            this._verifyElement(element);
            vl.log("Clicking the element");
     ...
     */
    async doubleClick(element, timeout = process.env.QMATE_CUSTOM_TIMEOUT || 30000) {
     ...
     */
    async rightClick(element, timeout = process.env.QMATE_CUSTOM_TIMEOUT || 30000) {
    ...
    async check(element) {
        ...
        try {
            this._verifyElement(element);
            const isSelected = await nonUi5.element.isSelected(element);
            if (!isSelected) {
     ...
     */
    async fillAndRetry(element, value, retries = 3, interval = 5000) {
     ...
     */
    async clearAndRetry(element, retries = 3, interval = 5000) {
     ...
     */
    async clearAndFillAndRetry(element, value, retries = 3, interval = 5000, verify = true) {
     ...
     */
    async mouseOverElement(element, xOffset, yOffset) {
     ...
     */
    async scrollToElement(element, alignment = "center") {
     ...
     */
    async dragAndDrop(element, targetElem) {
     ...
     */
    async moveCursorAndClick(element) {
     ...
     */
    async clickElementInSvg(svgElem, innerSelector) {
    ...
    _throwErrorForFunction(functionName, error) {
